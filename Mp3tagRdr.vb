...
#Region "Основа"
	...
	Structure MP3Tag
		Public Sub New(ByVal DefaultText As String)
			...
			Empty = False
		End Sub
		Public Sub Normalize(ByVal ConsiderYear As Boolean)
			If Album = "" Or Artist = "" Or (Year = "" And ConsiderYear) Or Number = "" Or Track = "" Then Valid = False
			If Year.Length = 3 Then Valid = False
			...
			End If
			If Track.StartsWith(" (") And Track.EndsWith(")") Then Track = Track.Substring(2, Track.Length - 3)
			If Track = "" Then Track = IO.Path.GetFileNameWithoutExtension(rs.Name)
		End Sub
		Public Sub CopyTo(ByRef t As MP3Tag)
			t.Album = Album
			t.Artist = Artist
			t.Comments = Comments
			t.Genre = Genre
	...
	Dim il As ID3Mode
	Function TestTAG(ByVal Filename As String, ByVal ID3Level As ID3Mode) As ParseMode
		host.LX("TestTAG: begin @ " & Filename & ", ID3Mode set to " & ID3Level)
		il = ID3Level
		Dim str As String
		rs = New IO.FileStream(Filename, IO.FileMode.Open, IO.FileAccess.Read)
		...
		host.LX("TestTAG: TAG subcription not found: " & str)
		rs.Seek(0, IO.SeekOrigin.Begin)
		str = Chr(rs.ReadByte) & Chr(rs.ReadByte) & Chr(rs.ReadByte)
		If str = "ID3" And ((ID3Level And ID3Mode.Level2) > 0) Then Return ParseMode.MP3
		host.LX("TestTAG: ID3 subcription not found: " & str)
		rs.Seek(0, IO.SeekOrigin.Begin)
		br = New IO.BinaryReader(rs, System.Text.Encoding.Default)
		Dim g As Guid
		g = readGUID() 'ВМА-файл начинается с ГУИД его, и если он не стандартный, то ... см. ниже
		If g = hdrGUID Then Return ParseMode.WMA
		host.LX("TestTAG: ASF Header GUID not found: " & g.ToString)
		rs.Seek(0, IO.SeekOrigin.Begin)
		str = Chr(rs.ReadByte) & Chr(rs.ReadByte) & Chr(rs.ReadByte)
		If str = "Ogg" Then Return ParseMode.OGG
		host.LX("TestTAG: Ogg subcription not found: " & str)
		rs.Seek(0, IO.SeekOrigin.Begin)
		str = Chr(rs.ReadByte) & Chr(rs.ReadByte) & Chr(rs.ReadByte) & Chr(rs.ReadByte)
		If str = "fLaC" Then Return ParseMode.FLAC
		host.LX("TestTAG: fLaC subcription not found: " & str)
		Return ParseMode.None
	End Function
	Public Function GenTag(ByVal Filename As String, ByVal ID3Level As ID3Mode, Optional ByVal ConsiderEmptyYearAsError As Boolean = False) As MP3Tag
		...
		ElseIf PM = ParseMode.FLAC Then
			GenTag = ReadFLACTag()
		Else
			GenTag = New MP3Tag("Tag not detected.")
			GenTag.Valid = False
			GenTag.Empty = True
		End If
		GenTag.Normalize(ConsiderEmptyYearAsError)
		host.LogLevel -= 1
...
#Region "Общие функции"
	Function Razor(ByVal text As String) As String
	...
	End Function
	Function ByteString2UtfBytesArray(ByVal input As String, ByVal SrcEncoding As System.Text.Encoding) As Byte()
	...
	End Function
	Function DecodeUTF(ByVal bytes() As Byte) As String
		...
		Return sb.ToString
	End Function
	Function DecodeUTF(ByVal Input As String) As String
		Return DecodeUTF(ByteString2UtfBytesArray(Input, Nothing))
	End Function
	Function DecodeUTF8(ByVal bytes() As Byte) As OGGVarPair
...
#Region "MP3"
	Function ReadMP3Tag() As MP3Tag
		...
		If (il And ID3Mode.Level1) > 0 Then
			...
			For k As Integer = 0 To 127
				Decoded &= Chr(Bytes128(k))
			Next
			...
			If Decoded.StartsWith("TAG") Then
				...
				If Bytes128(125) = 0 Then
					...
					Tag1.Number = Format(Bytes128(126), "00")
				Else
		...
		If (il And ID3Mode.Level2) > 0 Then
			...
			rs.Seek(0, IO.SeekOrigin.Begin)
			rs.Read(Id3Hdr, 0, 10)
			Id3Sz = CInt(Id3Hdr(6) * 128 ^ 3 + Id3Hdr(7) * 128 ^ 2 + Id3Hdr(8) * 128 + Id3Hdr(9)) '4 * %0xxxxxxx
			If Id3Sz > 0 And Id3Hdr(0) = &H49 And Id3Hdr(1) = &H44 And Id3Hdr(2) = &H33 Then '"ID3"
				...
				If Id3Hdr(3) < 5 And Id3Hdr(3) > 1 Then
					...
					Dim BytesTAG(Id3Sz - 1) As Byte, CharsTAG(Id3Sz - 1) As Char
					rs.Read(BytesTAG, 0, Id3Sz)
					host.LX("ReadMP3Tag: closing...")
					rs.Close()
					host.LX("ReadMP3Tag: working level2 tag...")
					host.LogLevel += 1
					For j As Integer = 0 To Id3Sz - 1
						CharsTAG(j) = Chr(BytesTAG(j))
					Next
					Decoded = New String(CharsTAG)
					...
					host.LX("ReadMP3Tag: level2 tag version = «" & Id3Hdr(3).ToString & "»")
					If Id3Hdr(3) = 2 Then
						NextFramePointer = 0
						host.LogLevel += 1
						...
						End While
					Else
						NextFramePointer = If((Id3Hdr(5) And &H40) > 0, CInt(BytesTAG(0) * 128 ^ 3 + BytesTAG(1) * 128 ^ 2 + BytesTAG(2) * 128 + BytesTAG(3)), 0)
		...
		End If
	End Function
	Enum ID3StringFmt As Byte
		ISO8859 = 0
		Unicode = 1
		UTF16BE = 2
		UTF8 = 3
	End Enum
	Dim sGenreMatrix() As String = ("Blues|Classic Rock|Country|Dance|Disco|Funk|Grunge|" & _
...
#Region "WMA"
	...
	Enum ECDON_DT
		...
		Word = 5
	End Enum
	Function GetObjData(ByVal Collection() As ExContDescrObjNrw, ByVal ObjName As String) As String
...
#End Region
#Region "OGG, FLAC"
	Function ReadOGGTag() As MP3Tag
		ReadOGGTag = New MP3Tag("")
	...
	End Function
	Function ReadFLACTag() As MP3Tag
		ReadFLACTag = New MP3Tag("")
		Dim found As Boolean = False
	...
	End Function
	Function ParseVorbisComment() As MP3Tag
		ParseVorbisComment = New MP3Tag("")
